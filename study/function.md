...
배열의 값을 한 개로 감소 시키는 특징을 갖고 있음. map, filter, find 함수로 구현할 수 있는 기능은 모두 reduce함수로 구현 가능하다.

```javascript
[].reduce(callback[,initialValue])
callback
1. previousValue : 마지막 콜백에서 반환된 값 또는 initialValue
2. currentValue : 현재 배열 내 처리되고 있는 요소
3. currentIndex : 현재 배열 내 처리되고 있는 요소의 인덱스
4. array : reduce 호출에 사용되는 원 배열

initialValue : callback의 첫 번째 인수의 값에 사용되는 값

// example1
const arr = [12, 4, 19, 33, 86]
const sum = arr.reduce((pre,value,idx,arr) => {
    return pre + value;
});
